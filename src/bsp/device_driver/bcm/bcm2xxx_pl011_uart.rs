...
// Api explanation can be found here: https://github.com/tock/tock/tree/master/libraries/tock-register-interface
register_bitfields! {
  u32,
  ...
  FR [
    ...
    /// FEN bit in the Line Control Register, UARTLCR_LCRH.
    ///
    ...
    ///
    /// If the FIFO is disabled, this bit is set when the transmit holding register
    /// is full. If the FIFO is enabled, the TXFF bit is set when the transmit
    /// FIFO is full.
    ...
    /// is empty. If the FIFO is enbaled, the RXFE bit is set when the receive
    /// FIFO is empty.
  ...
  CR [
    ...
    /// is enabled. Data reception occurs for UART signals. When the UART is disabled
    ...
    /// enabled. Data transmission occurs for UART signals. When the UART is disabled in
    /// the middle of transmission, it completes the current character before stopping.
...
impl PL011UartInner {
    ...
    ///
    /// The calculation for the BRD given a target rate of 230400 and a clock set to
    /// 48 MHz is: `(48_000_000/16)/230_400 = 13,02083`. `13`goes to the `IBRD`
    /// (integer field). The `FBRD` (fractional field) is only 6 bots so
    /// `0,0208*64 = 1,3312 rounded to 1` will give the best approximation we can get.
    /// A 5 modulo error margin is acceptable for UART and we're now at 0,01 modulo.
    ///
    /// This results in 8N1 and 230_400 baud (we set the clock in config.txt).
...
/// in turn are used to implement the `kernel`'s `print!` and `println!` macros. By
/// implementing `write_str()`, we get `write_fmt()` automatically.
///
