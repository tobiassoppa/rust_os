...
unsafe extern "C" fn current_elx_synchronous(e: &mut ExceptionContext) {
    ...
    // advances the exception link register for one instruction, so that the execution can continue.
...
impl fmt::Display for EsrEL1 {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    // Raw print of instruction specific syndome.
    write!(f, "     Instr Specific Syndrom (ISS): {:#x}", esr_el1.read(ESR_EL1::ISS))?;
...
impl fmt::Display for SpsrEL1 {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    writeln!(f, "             Zero      (N): {}", to_flag_str(self.0.is_set(SPSR_EL1::Z)))?;
    writeln!(f, "             Carry     (N): {}", to_flag_str(self.0.is_set(SPSR_EL1::C)))?;
    writeln!(f, "             Overflow  (N): {}", to_flag_str(self.0.is_set(SPSR_EL1::V)))?;
...
pub fn current_privilege_level() -> (PrivilegeLevel, &'static str) {
    ...
    match el {
        ...
        Some(CurrentEL::EL::Value::EL1) => (PrivilegeLevel::Kernel, "EL2"),
        Some(CurrentEL::EL::Value::EL0) => (PrivilegeLevel::User, "EL2"),
