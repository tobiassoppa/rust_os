...
/// A table descriptor for the 64 KiB aperture.
///
...
/// A page descriptor for the 64 KiB aperture.
///
...
/// Big monolithic struct for storing the translation tables. Individual levels
/// must be 64 KiB aligned, hence the "revers" order of appearance.
#[repr(C)]
#[repr(align(65536))]
...
/// Usually evaluates to 1 GiB for RPi3 and 4 GiB for RPi4.
...
///
/// # Safety
/// - Suppsed to land in `.bss`. Therefore, ensure that all initial member values
/// boil down to "0".
...
unsafe fn populate_tt_entries() -> Result<(), &'static str> {
    for (l2_nr, l2_enty) in KERNEL_TABLES.lvl2.iter_mut().enumerate() {
        *l2_enty = KERNEL_TABLES.lvl3[l2_nr].base_addr_usize().into();
...
fn configure_translation_control() {
    ...
    TCR_EL1.write(
        TCR_EL1::TBI0::Ignored
            ...
            + TCR_EL1::T0SZ.val(32), // TTBR0 spans 4 GiB in total.
    );
}
...
impl memory::mmu::interface::MMU for MemoryManagementUnit {
    unsafe fn init(&self) -> Result<(), &'static str> {
        ...
        // Forche MMU init to complete before next instruction.
        barrier::isb(barrier::SY);
